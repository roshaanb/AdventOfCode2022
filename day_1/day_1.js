...
function countHighestCalories(filename) 
    ...
    const arrayOfCaloriesSeparatedByElf = rawData.split("\n").toString().split(",,");

    // creating array of sum of each elf's calories 

    const elfCaloriesSummed = arrayOfCaloriesSeparatedByElf.map(stringOfNums => {
         ...
         return sum = 
         numsToInts.reduce((accumulator, value) => {
    ...
    elfCaloriesSummed.sort(function(a, b){return b-a});

    const elfWithMost = elfCaloriesSummed[0]
    const sumOfHighest3 = elfCaloriesSummed[0] + elfCaloriesSummed[1] + elfCaloriesSummed[2]
